...
Вызывается, когда пользователь не просил фан-аут явно. Показывает дешёвой модели
последние сообщения пользователя и паспорт предлагаемого workflow, и получает ответ,
оправдан ли запуск. Асимметрия зашита в промпт: отказ дёшев (Клод сделает работу сам),
ошибочное разрешение стоит пользователю сотен тысяч токенов.
"""
import json
import os
import re
